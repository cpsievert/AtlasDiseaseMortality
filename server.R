...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$broad_selected999, {
    ...
    if("Broad 10 categories of disorders" %in% x) {
      list <- list%>% bind_rows(list_unique999 %>% filter(id %in% (id_broad))) %>% distinct()
    }
    
    if("Specific 31 disorders" %in% x) {
      list <- list%>% bind_rows(list_unique999 %>% filter(id %in% (id_specific))) %>% distinct()
      if("Broad 10 categories of disorders" %in% x) {
        list <- list %>% 
          mutate(
  ...
  observeEvent(input$btn_list_disorders, {
    # Show a simple modal
    shinyalert(title = "List of disorders", type = "info", 
  ...
  observeEvent(input$button_main999, {
    hide_show_arrows(button = "button_main999", button2 = input$button_main999, panel = "main_panel999", session = session,
  ...
  observeEvent(input$button_agedx999, {
    show_hide_arrows(button = "button_agedx999", button2 = input$button_agedx999, panel = "agedx999_show", session = session,
  ...
  observeEvent(input$button_agedeath999, {
    show_hide_arrows(button = "button_agedeath999", button2 = input$button_agedeath999, panel = "agedeath999_show", session = session,
  ...
  observeEvent(input$button_lagged999, {
    show_hide_arrows(button = "button_lagged999", button2 = input$button_lagged999, panel = "lagged999_show", session = session,
  ...
  observeEvent(input$button_LYLages999, {
    show_hide_arrows(button = "button_LYLages999", button2 = input$button_LYLages999, panel = "LYLages999_show", session = session,
  ...
  observeEvent(input$button_MRR999, {
    show_hide_arrows(button = "button_MRR999", button2 = input$button_MRR999, panel = "MRR999_show", session = session,
...
#  output$MRR_LYL_causes999 <- renderTable({
  ...
  observeEvent(input$button_survival999, {
    show_hide_arrows(button = "button_survival999", button2 = input$button_survival999, panel = "survival999_show", session = session,
                     text = "age-specific and cause-specific survival curves")
  })

  observeEvent(input$dx_interest999, {
    
    x <- list_dx %>% filter(description == gsub("- ", "", gsub("Additional: ", "", !!input$dx_interest999)), LYL == TRUE) %>%
      slice(1)
  ...
  observeEvent(input$button_air999, {
    show_hide_arrows(button = "button_air999", button2 = input$button_air999, panel = "air999_show", session = session,
  ...
  observeEvent(input$button_MRRLYLall999, {
    ...
    hide_show_arrows(button = "button_MRRLYLall999", button2 = input$button_MRRLYLall999, panel = "MRRLYL_all999_show", session = session,
  ...
  observeEvent(input$button_MRRLYLall, {
    ...
    hide_show_arrows(button = "button_MRRLYLall", button2 = input$button_MRRLYLall, panel = "MRRLYL_all_show", session = session,
                     text = "a panel of number of diagnosed, mortality rate ratios and life years lost")
  ...
  observeEvent(check_plot_all999(), {
    ...
    output$MRR_LYL_all999 <- renderPlot({
      plot_all(dx = x, sex = sex, causes = input$cod999, list_dx_included = input$diseases_all999, 
               list_clean = gsub("- ", "", gsub("Additional: ", "", input$diseases_all999)))
    })
    
    output$MRR_all999 <- renderPlot({
      plot_mrr_all(dx = x, sex = sex, causes = input$cod999, list_dx_included = input$diseases_all999, 
                   list_clean = gsub("- ", "", gsub("Additional: ", "", input$diseases_all999)))
    })
    
    output$LYL_all999 <- renderPlot({
      plot_lyl_all(dx = x, sex = sex, causes = input$cod999, list_dx_included = input$diseases_all999, 
                   list_clean = gsub("- ", "", gsub("Additional: ", "", input$diseases_all999)))
    })

    output$MRR_air_plot999 <- renderPlot({
      plot_mrr_air(dx = x, list_dx_included = input$diseases_all999, 
                   list_clean = gsub("- ", "", gsub("Additional: ", "", input$diseases_all999)))
    })
  ...
  observeEvent(check_plot_all(), {
    ...
    if("All" %in% sex) {
      out <- estimates_sex %>% filter(include == 0) %>% select (id) %>% left_join(
        estimates %>% filter(sex != "All") %>% filter(include == 1), by="id"
      )
    ...
    output$MRR_LYL_all <- renderPlot({
      plot_all(dx = x, sex = sex, causes = input$cod, list_dx_included = labels$lab, 
               list_clean = labels$description)
    })
    
    output$MRR_all <- renderPlot({
      
      g0 <-      plot_mrr_all(dx = x, sex = sex, causes = input$cod, list_dx_included = labels$lab, 
                              list_clean = labels$description)
      ...
      if(nrow(shaded) > 0) {
        g <- g + geom_rect(data = shaded, inherit.aes = FALSE,
    ...
    output$LYL_all <- renderPlot({
      g0 <- plot_lyl_all(dx = x, sex = sex, causes = input$cod, list_dx_included = labels$lab, 
                   list_clean = labels$description)
    ...
    output$MRR_air_plot <- renderPlot({
      g0 <- plot_mrr_air(dx = x, list_dx_included = labels$lab, 
                   list_clean = labels$description)
  ...
  observeEvent(input$button_MRRall999, {
    show_hide_arrows(button = "button_MRRall999", button2 = input$button_MRRall999, panel = "MRR_all999_show", session = session,
                     text = "mortality rate ratios only")

  })
  
  observeEvent(input$button_MRRall, {
    show_hide_arrows(button = "button_MRRall", button2 = input$button_MRRall, panel = "MRR_all_show", session = session,
                     text = "mortality rate ratios only")
  ...
  observeEvent(input$button_LYLall999, {
    show_hide_arrows(button = "button_LYLall999", button2 = input$button_LYLall999, panel = "LYL_all999_show", session = session,
                     text = "life years lost only")

  })
  
  observeEvent(input$button_LYLall, {
    show_hide_arrows(button = "button_LYLall", button2 = input$button_LYLall, panel = "LYL_all_show", session = session,
                     text = "life years lost only")
  ...
  observeEvent(input$button_airall999, {
    show_hide_arrows(button = "button_airall999", button2 = input$button_airall999, panel = "air_all999_show", session = session,
                     text = "mortality rate ratios after adjustment for air pollution")

  })

  observeEvent(input$button_airall, {
    show_hide_arrows(button = "button_airall", button2 = input$button_airall, panel = "air_all_show", session = session,
                     text = "mortality rate ratios after adjustment for air pollution")
...
#  output$MRR_air_plot999 <- renderPlot({
 ...
 #   plot_mrr_air(dx = gsub("- ", "", gsub("Additional: ", "", input$diseases_all999)))
  ...
  observeEvent(input$button_main, {
    hide_show_arrows(button = "button_main", button2 = input$button_main, panel = "main_panel", session = session,
                     text = "table with main results")
  })
  ...
  observeEvent(input$button_agedx, {
    show_hide_arrows(button = "button_agedx", button2 = input$button_agedx, panel = "agedx_show", session = session,
                     text = "age-specific incidence of the disorder and distribution of age of diagnosis")
  })
  ...
  observeEvent(input$button_agedeath, {
    show_hide_arrows(button = "button_agedeath", button2 = input$button_agedeath, panel = "agedeath_show", session = session,
                     text = "age-specific mortality rates and distribution of age of death")
  })
  ...
  observeEvent(input$button_lagged, {
    show_hide_arrows(button = "button_lagged", button2 = input$button_lagged, panel = "lagged_show", session = session,
                     text = "mortality rate ratios depending on age and time since diagnosis")
  })
  ...
  observeEvent(input$button_LYLages, {
    show_hide_arrows(button = "button_LYLages", button2 = input$button_LYLages, panel = "LYLages_show", session = session,
                     text = "age-specific remaining life expectancy")
  })  
  ...
  observeEvent(input$button_MRR, {
    show_hide_arrows(button = "button_MRR", button2 = input$button_MRR, panel = "MRR_show", session = session,
                     text = "mortality due to specific causes of death")
  })
  ...
  observeEvent(input$button_survival, {
    show_hide_arrows(button = "button_survival", button2 = input$button_survival, panel = "survival_show", session = session,
                     text = "age-specific and cause-specific survival curves")
  })
  
  observeEvent(input$dx_interest, {
    
    x <- list_dx %>% filter(description == !!input$dx_interest, LYL == TRUE) %>%
      slice(1)
  ...
  observeEvent(input$button_air, {
    show_hide_arrows(button = "button_air", button2 = input$button_air, panel = "air_show", session = session,
                     text = "mortality rate ratios after adjustment for air pollution")
  })
