...
plot_incidence <- function(dx) {
  x <- list_dx %>% filter(description == !!dx) %>%
    select(id) %>% distinct() %>% left_join(incidence, by="id") %>%
    filter(age_group <= 95)
  ...
  if(dx != "--> Choose disorder of interest") {
    g <- ggplot(x, aes(x=age_group+2.5, y=10000*dx_rate))+
      ...
      xlab("Age in years") + ylab("Incidence rate (per 10,000 person-years)")+
...
plot_mortality <- function(dx) {
  x <- list_dx %>% filter(description == !!dx) %>%
    select(id) %>% add_row(id=0) %>% distinct() %>% left_join(incidence, by="id") %>%
    mutate(
      id = ifelse(id==0, 0, 1),
  ...
  if(dx != "--> Choose disorder of interest") {
    g <- ggplot(x, aes(x=age_group+2.5, y=10000*death_rate, group=factor(id), color=factor(id)))+
      ...
      xlab("Age in years") + ylab("Mortality rates (per 10,000 person-years)")+
      scale_x_continuous(limits=c(0, 100), breaks = c(0, 25, 50, 75, 100))+
      facet_grid(~sex, drop = FALSE)+
      theme_bw()+
      scale_color_manual(
...
plot_age_dx <- function(dx, smooth) {
  x <- list_dx %>% filter(description == !!dx) %>%
    select(id) %>% distinct() %>% left_join(ages, by="id")
  ...
  if(dx != "--> Choose disorder of interest") {
    ...
    if(smooth == FALSE) {
      g <- ggplot(x, aes(x=age, y=density_dx))+
        ...
        xlab("Age in years") + ylab("Distribution of age of diagnosis (density)")+
        scale_x_continuous(limits=c(0, 100), breaks = c(0, 25, 50, 75, 100))+
        facet_grid(~sex, drop = FALSE)+
        theme_bw()
    } else {
  ...
  if(dx != "--> Choose disorder of interest") {
    ...
    if(smooth == FALSE) {
      g <- ggplot(x, aes(x=age, y=density_death))+
        geom_line(color="black")+
        xlab("Age in years") + ylab("Distribution of age of death among the diagnosed (density)")+
        scale_x_continuous(limits=c(0, 100), breaks = c(0, 25, 50, 75, 100))+
        facet_grid(~sex, drop = FALSE)+
        theme_bw()
    } else {
...
plot_MRRlagged <- function(dx) {
  ...
  if(dx != "--> Choose disorder of interest") {
    ...
    x <- list_dx %>% filter(description == !!dx) %>%
      select(id) %>% distinct() %>% left_join(MRRlagged, by="id") %>%
      filter(exposure!=0)
    ...
    g1 <- ggplot(x, aes(x=exposure, y=HR))+
      ...
      scale_y_log10()+
      xlab("Time since first diagnosis") + ylab("Mortality rate ratio (95% CI)")+
      theme_bw()
    
    x <- list_dx %>% filter(description == !!dx) %>%
      select(id) %>% distinct() %>% left_join(MRRage, by="id") %>%
    ...
    g2 <- ggplot(x, aes(x=age_group, y=HR))+
      ...
      geom_line(linetype="dashed")+
      facet_grid(~sex, drop = TRUE)+
      scale_x_continuous(breaks=seq(0, 95, age_categories), 
                         labels=paste0(seq(0, 95, age_categories), "-",seq(age_categories, 95+age_categories, age_categories))
      )+
      scale_y_log10()+
      xlab("Age in years") + ylab("Mortality rate ratio (95% CI)")+
      theme_bw()+
      theme(
...
table_causes <- function(dx) {
  ...
  if(LYLs > 0) {
    ...
    t <- t1 %>%
      ...
      select('Sex' = sex, 'Cause of death' = cod, 'Mortality rates diagnosed' = rate, 'Mortality rates Denmark' = rateDK_std,
...
table_MRRair <- function(dx) {
  t <- list_dx %>% filter(description == !!dx) %>%
    ...
    select(-id) %>%
    mutate(empty = "") %>%
    select('Sex' = sex, rate, rateDK_std, empty,
...
plot_LYLages <- function(dx) {
  x <- list_dx %>% filter(description == !!dx) %>%
    select(id) %>% distinct() %>% left_join(LYLages, by="id")
  
  x <- x %>% select(-life_exp0) %>% mutate(group = "Diagnosed with the disorder") %>% bind_rows(
...
plot_LYLplot <- function(dx, age, cause) {
  if(dx != "--> Choose disorder of interest") {
    ...
    g <- ggplot(LYLplot2, aes(x=time, y=100*cip, fill = cause))+
      ...
      xlab("Age in years") + ylab("Percentage of persons alive")+
      theme_bw()+
    ...
    check <- list_dx %>% filter(description == !!dx) %>%
      filter(LYL == TRUE) %>%
      select(id, p25, p50, p75) %>% distinct()
    point <- seq(0, 100, 10)
    ...
    causes <- FALSE
    text <- paste0("<b>Interpretation:</b> When focusing on one specific age, it is possible to look at the survival curves from that age, as well as the cumulative incidences of dying. 
    ...
    if(nrow(LYL_plot_sex) > 0) {
      ...
      text <- paste0(text, " When looking at ", tolower(sex), " with a diagnosis at age ", min_age, " years, ", surv, "% of them are alive at age ", point, 
      ...
      if("Dead (natural causes)" %in% LYL_plot_sex$cause) {
        ...
        text <- paste0(text, " More specifically, ", nat, "% die of natural causes and ", ext, "% of external causes (compared to ",
      ...
      LYL_plot_sex <- LYL_plot_sex %>%
        ...
        select(id, sex) %>% distinct() %>% left_join(LYLages, by=c("id", "sex")) %>%
        filter(age == !!min_age)
      
      text <- paste0(text, " When looking at remaining life expectancy (corresponding to the area under the survival curve), ", tolower(sex), 
      ...
      if(causes == TRUE) {
        text <- paste0(text, ", which can be decomposed into ", format_numbers(LYL_plot_sex$LYL_Natural, 1), " years due to natural causes and ", 
...
interpretation_main <- function(dx) {
  ...
  N_text <- paste0("<b>Interpretation:</b> A total of ", N[1, "Diagnosed"], " ", sex, " living in Denmark in 2000-2018 were diagnosed for the first time with disorder \"", N[1, "desc_EN"],
                   "\" in a hospital in 1995-2018.")
  
  age <- N[1, "age_dx50"]
  if(!is.na(age)) {
    N_text <- paste0(N_text, " The median age at diagnosis was ", format_numbers(age, 1), " years, while 25% of the diagnosed were 
  ...
  age <- N[1, "age_death50"]
  if(!is.na(age)) {
    N_text <- paste0(N_text, ", and the median age at time of death was ", format_numbers(age, 1), " years (25% were 
  ...
  if(nrow(x) != 0) {
    ...
    text_incidence <- paste0("<b>Interpretation:</b> These figures show the age-specific incidence rates (per 10,000 person-years) of being diagnosed for the first time with disorder \"", N[1, "desc_EN"],
    ...
    if(age == age2) {
      ...
      text_incidence <- paste0(text_incidence, ", while it will be ", rate, " among ", rate_sex, " of age ", age2, "-", age2+5, " years  (IR = ", rate, " (95% CI: ", rate_left,
                          "-", rate_right, ")).")
    }
  } else {
    text_incidence <- ""
  ...
  age <- N[1, "age_dx50"]
  if(!is.na(age)) {
    incidence2 <- paste0(incidence2, " The median age at diagnosis for ", sex, " was ", format_numbers(age, 1), " years, while 25% of the diagnosed were 
                     younger than ", format_numbers(N[1, "age_dx25"], 1), " years and 25% were older than ", format_numbers(N[1, "age_dx75"], 1), 
                     " years at time of diagnosis.")
  } 
  ...
  if(nrow(x) != 0) {
    ...
    mortality <- paste0("<b>Interpretation:</b> These figures show the age-specific mortality rates (per 10,000 person-years) among those diagnosed for the first time with disorder \"", N[1, "desc_EN"],
    ...
    if(age == age2) {
      ...
      mortality <- paste0(mortality, ", while an average of ", rate, " deaths are expected to be observed if 10,000 ", rate_sex, " of age ", age2, "-", age2+5, " years diagnosed for the first time with disorder \"", N[1, "desc_EN"],
                          "\" are followed for one year (MR = ", rate, " (95% CI: ", rate_left,
                          "-", rate_right, ")), compared to ", rate0, " in the general population of same age (MR = ", rate0, " (95% CI: ", rate_left0,
                          "-", rate_right0, ")).")
    }
    ...
    mortality2 <- paste0("<b>Interpretation:</b> These figures show the distribution of age-of-death among those diagnosed for the first time with disorder \"", N[1, "desc_EN"],
    "\" who died during the follow-up, without taking into account whether mortality was likely or unlikely.")
    
    age <- N[1, "age_death50"]
    if(!is.na(age)) {
      mortality2 <- paste0(mortality2, " Among those ", sex, " who died with a previous diagnosis, the median age at time of death was ", format_numbers(age, 1), " years (25% were 
                     younger than ", format_numbers(N[1, "age_death25"], 1), " years and 25% were older than ", format_numbers(N[1, "age_death75"], 1), 
                           " years).")
    } 
    ...
    lagged <- paste0("<b>Interpretation:</b> The left figure shows the mortality rate ratios comparing those with and without a diagnosis of disorder \"", N[1, "desc_EN"],
    ...
    if(!is.na(MRR_age)) {
      ...
      lagged <- paste0(lagged, "compared to those of same age and sex without that diagnosis (MRR = ", format_numbers(MRR_age, 2), " (95% CI: ", format_numbers(CI_left, 2),
    ...
    if(!is.na(MRR_age)) {
      ...
      lagged <- paste0(lagged, "compared to those without that diagnosis (MRR = ", format_numbers(MRR_age, 2), " (95% CI: ", format_numbers(CI_left, 2),
    ...
    if(nrow(x) > 0) {
      lagged <- paste0(lagged, " The right figure shows the mortality rate ratios comparing those with and without a diagnosis of disorder \"", N[1, "desc_EN"],
                       "\" depending on age. ")
      ...
      if(nrow(x) == 1) {
        ...
        lagged <- paste0(lagged, "at age ", age, "-", age+age_categories, " years compared to those of same age and sex without that diagnosis (MRR = ", format_numbers(MRR_age, 2), " (95% CI: ", format_numbers(CI_left, 2),
                         "-", format_numbers(CI_right, 2), ")).")
      }
    }
    ...
    ## LYL ages
    x <- list_dx %>% filter(description == dx) %>%
      select(id, p5, p95) %>% filter(!is.na(p5)) %>% distinct() %>% left_join(LYLages, by="id")
    ...
    if(nrow(x) > 0) {
      ...
      LYLages <- paste0("<b>Interpretation:</b> This figure shows the remaining life expectancies for each single age of a range that includes 90% of the diagnoses: from percentile 5 (",
      ...
      LYLages <- paste0(LYLages, "compared to ",
    ...
    y <- x %>% select(sex) %>% distinct() %>% mutate(cod="All") %>% left_join(
      t, by=c("sex", "cod")
    )
...
mortality rate ratios for all-cause mortality have been estimated after further adjustment for air pollution. This table shows mortality rates (per 1,000 person-years) for those diagnosed for the first time with disorder \"", N[1, "desc_EN"], "\" and the age- and sex-standardized
  ...
  return(list(main=main, incidence=text_incidence, incidence2=incidence2, mortality=mortality, mortality2=mortality2,
...
plot_all <- function(dx, sex, causes, list_dx_included, list_clean) {
  ...
  if("All" %in% sex) {
    out <- x_sex %>% filter(include == 0) %>% select (id) %>% left_join(
      dx %>% filter(sex != "All") %>% filter(include == 1), by="id"
    )
  ...
  x <- dx %>% select(id, sex, level, chapter, desc_EN, pos) %>% left_join(
    MRR, by=c("id", "sex")
  ) %>%
    filter(cod == !!causes)
  ...
  x2 <- dx %>% select(id, sex, level, chapter, desc_EN, pos) %>% left_join(
    LYL_aux, by=c("id", "sex")
  ) %>%
    filter(cod == !!causes)
...
plot_mrr_air <- function(dx, list_dx_included, list_clean) {
  ...
  x <- dx %>% select(id, sex, level, chapter, desc_EN, pos) %>% left_join(
    MRR %>%filter(cod %in% c("Air_adjusted", "Air_not_adjust")), by=c("id")
  ) %>%
    mutate(
